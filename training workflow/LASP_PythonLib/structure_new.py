...
class Str(object):
    ...
    def addStress(self, line, flag=1): # type 1 => for arc file; 2 => Data file
    ...
    def calAtomnum(self, ):
        ...
        #print(self.atom)
        #print(self.natom)
        #self.eleList,self.natompe = list(np.unique([atom.ele for atom in self.atom],return_counts=True))
        self.eleList=list(np.unique([atom.ele for atom in self.atom]))
        #print(self.eleList)
        self.natompe =[]
        for atom in self.eleList:
            int_atom=int(atom)
            atom_list=[atom.ele for atom in self.atom]
            atom_num=atom_list.count(int_atom)
            self.natompe.append(atom_num)
        #print(self.natompe)
            ...
            for i in range(self.Nat) :
                 if i not in self.centerf.keys():
                     ...
                     for j in self._currentlist:
                         if self.EleNam[j] in elelist.keys() : elelist[self.EleNam[j]] +=1
                         else: elelist[self.EleNam[j]] = 1
...
#               if self.sporder[self.EleNam[i]]>self.sporder[self.EleNam[j]]: bondname=self.EleNam[j]+"-"+self.EleNam[i]
                if PT.Eledict[self.EleNam[i]]>PT.Eledict[self.EleNam[j]] : bondname=self.EleNam[j]+"-"+self.EleNam[i]
                bonddict[bondname]=10.0
                #return bonddict
        for i in range(N):
...
#       return (qval[0], qval[1], qval[2])
    ...
    def SteinhartQ_cal(self):
        ...
        q = ctypes.cdll.LoadLibrary('/home11/Liugroup/tools/pybasics/calQ.so')
        xa = reduce(lambda a,b:a+b, self.Coord)
        cell = reduce(lambda a,b:a+b, self.Cell)
